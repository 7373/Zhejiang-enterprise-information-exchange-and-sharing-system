...
require([
    ...
    var regState = function(data){
        var txt = '-';
        if (data=="K"||data=="B"||data=="A"||data=="DA"||data=="X")return txt = '存续';
        if (data == 'C')return txt = '撤销';
        if (data == 'D')return txt = '吊销';
        if (data == 'Q')return txt = '迁出';
        if (data=="XX"||data=="DX")return txt = '注销';
        return txt;
    };
            ...
            columnDefs : [
                {
                    ...
                    render: function (data, type, row, meta) {
                        return "<a href='javascript:void(0)' class='mr10 js-view' target='_blank'>查看详情</a></a>";
                    }
                },
                {
                    targets: 2,
        ...
        util.bindEvents([
            {
                ...
                handler: function () {
                   ...
                   /* layer.dialog({
                        title: '文书回执详情',
                        area: ['95%', '95%'],
                        content: '/reg/server/write/writDetails?priPID='+priPID+"&noticePeople="+noticePeople+"&tel="+tel+"&uuid="+uuid
                    });*/
