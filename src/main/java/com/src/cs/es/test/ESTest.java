...
//        request.setSql("select count(*) from anls_cs_mid_baseinfo_pub");//索引查询sql
        //request.setSql("select count(*) from anls_cs_idx_panorama where yrIsRep2015='0'");
    ...
    public static void testExcuteIndexInsert() throws ApiException{
        ...
        request.setIndex("anls_test");//索引名
        request.setKey("4444");//主键_ID的值
        request.setJsonData("{\"dff\":\"test\"}");//要插入的字段名和字段值
    ...
    public static void testSqlDeleteIndexRequest() throws ApiException {
        IndexDeleteRequest indexDeleteRequest = new IndexDeleteRequest();
        indexDeleteRequest.setSql("delete from anls_cs_idx_panorama where priPID = '3309090000003881'");
        IndexDeleteResponse response = client.excute(indexDeleteRequest);
