...
public class PubScentBackServiceImpl extends MyBatisServiceSupport implements IPubScentBackService {
	...
	@Override
	public boolean doAddBatch(PubScentBackDto PubScentBackDto) throws Exception {
		if(CollectionUtils.isNotEmpty(PubScentBackDto.getPriPIDs())){
			for(String priPID : PubScentBackDto.getPriPIDs()){
				if(checkExist(PubScentBackDto.getTaskUid(), priPID)){
					savePubScentBack(priPID,PubScentBackDto.getTaskUid());
				}
			}
			return true;
		}
...
//			int checkNum = pubScdeptTask.getCheckNum();
	...
	@Transactional(rollbackFor=Exception.class)
	@Override
	public JSONObject doAddExcelBatch(InputStream in, String taskUid) throws Exception {
		JSONObject json = new JSONObject();
		String message = "";
		int sucNum = 0;
		int failNUm = 0;
	...
	@Transactional(rollbackFor=Exception.class)
	@Override
	public boolean doCleanPubScentBack(String taskUid) throws Exception {
	...
	@Transactional(rollbackFor=Exception.class)
	@Override
	public boolean doRandomEntBack(SysUserDto sysUser,String taskUid, String randomType) throws Exception {
		...
		if(CollectionUtils.isNotEmpty(pubScentConditionList) && pubScentConditionList.size() > 0){
			...
			for(PubScentCondition pubScentCondition : pubScentConditionList){
				...
				if("1".equals(pubScentCondition.getCheckType())){
					...
					randomSpecial(pubScentCondition,taskUid);
				}
			}
	...
	private boolean randomMain(PubScentCondition pubScentCondition,String taskUid)throws Exception {
		...
		if(pubScentCondition.getSpecialNum() > 0 && pubScentCondition.getRandomNum() > 0){
			...
			if(entTotal <= randomNum){
				Example example=new Example(PubScentBack.class);
		        example.createCriteria().andEqualTo("taskUid",taskUid).andEqualTo("specialCode",pubScentCondition.getSpecialCode());
		        List<PubScentBack> pubScentBackList = pubScentBackMapper.selectByExample(example);
				...
				if(range >= 2){//跳号抽取
					...
					for(int i=0;i<randomNum;i++){
						if(StringUtils.isNotEmpty(pubScentCondition.getSpecialCode())){
							Map<String,Object> params = new HashMap<String,Object>();
							params.put("taskUid", taskUid);
							params.put("limit", firstNum);
							PubScentBack pubScentBack = pubScentBackMapper.selectPubScentBackForRandom(params);
							pubScentService.savePubScent(pubScentBack);
					...
					if(range == 1 && mol > 0){
						...
						for(int i = 0;i < randomNum;i++){
			    			boolean flag = true;
			    			while(flag){
			    				int rand = new Random().nextInt(entTotal)+1;
			    				int entNumber = rand - 1;
			    				Map<String,Object> params = new HashMap<String,Object>();
								params.put("taskUid", taskUid);
								params.put("limit", entNumber);
								PubScentBack pubScentBack = pubScentBackMapper.selectPubScentBackForRandom(params);
			    				if(pubScentBack !=null && StringUtils.isNotEmpty(pubScentBack.getPriPID())){
			    					String priPID = pubScentBack.getPriPID();
	 ...
	 * @param pubScentCondition
	 */
	private boolean randomSpecial(PubScentCondition pubScentCondition,String taskUid)throws Exception {
		//1.判断是否需要抽取
		if(pubScentCondition.getSpecialNum() > 0 && pubScentCondition.getRandomNum() > 0){
			int entTotal = pubScentCondition.getSpecialNum();//该专项库对应的企业数量
			int randomNum = pubScentCondition.getRandomNum();//该专项库对应的需要抽取企业数量
			//2.全部抽取
			if(entTotal <= randomNum){
				List<PubScentSpecial> pubScentSpecialList = pubScentSpecialService.selectScentSpecialList(taskUid,pubScentCondition.getCheckCode());
		        for(PubScentSpecial pubScentSpecial : pubScentSpecialList){
		        	pubScentService.savePubScent(pubScentSpecial);
		        }
			}else{
				//3.计算随机生成数区间
				int range = entTotal/randomNum;
				...
				if(range >= 2){//跳号抽取
					...
					for(int i=0;i<randomNum;i++){
						if(StringUtils.isNotEmpty(pubScentCondition.getSpecialCode())){
							...
							params.put("limit", firstNum);
							PubScentSpecial pubScentSpecial = pubScentSpecialService.selectPubScentSpeForRandom(params);
							pubScentService.savePubScent(pubScentSpecial);
							firstNum = firstNum + range;
						}
					}
					...
					if(range == 1 && mol > 0){
						...
						for(int i = 0;i < randomNum;i++){
			    			...
			    			while(flag){
								...
								params.put("limit", entNumber);
								PubScentSpecial pubScentSpecial = pubScentSpecialService.selectPubScentSpeForRandom(params);
			    				if(pubScentSpecial !=null && StringUtils.isNotEmpty(pubScentSpecial.getPriPID())){
	...
	@Transactional(rollbackFor=Exception.class)
	@Override
	public boolean doCleanPubScentBack(List<String> entBackUidList)throws Exception {
